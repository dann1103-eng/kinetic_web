  ...
  if (rows.length === 0) {
    return (
      <section className="rounded-3xl border border-emerald-200 bg-emerald-50/50 p-6">
        ...
        <p className="text-sm text-emerald-800/80 mt-2 max-w-prose">
    ...
    <section className="rounded-3xl border border-amber-200 bg-amber-50/40 p-6">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          ...
          <h2 className="text-xl font-semibold text-amber-950 mt-1">
            Niños en riesgo
            <span className="ml-2 text-sm font-medium text-amber-800/70 tabular-nums">
              {rows.length}
            </span>
          </h2>
        </div>
      ...
      <ul className="space-y-2">
        {rows.map((c) => (
          <li
            key={c.childId}
            className="rounded-2xl bg-fm-surface-container-lowest border border-amber-200/40 px-4 py-3"
          >
            <Link
            ...
            >
              ...
              <span
                className="material-symbols-outlined text-amber-700 group-hover:text-fm-primary shrink-0 transition-colors"
                aria-hidden="true"
              >
                arrow_outward
              </span>
