-- =============================================================================
-- 0139 — Rollover de terapias no dadas al crear el ciclo siguiente
-- =============================================================================
-- Al generar el ciclo, recepción puede arrastrar las sesiones del mes anterior
-- que NO se dieron ni se repusieron. Dos modalidades:
--   • 'discount'   → descuento en la factura = Σ(sesiones no dadas × precio).
--                    (citas normales; pagan menos)
--   • 'accumulate' → se generan citas EXTRA ese mes (cuota subida en compute),
--                    sin recobrar — ya pagaron el mes anterior.
--
-- Cambios:
--   1) Columnas de rollover en monthly_session_cycles.
--   2) compute: nuevo arg opcional p_rollover_sessions (service→extra) que sube
--      la cuota por servicio (solo afecta cuántos candidatos se conservan).
--   3) confirm: nuevos args de rollover; pasa el rollover a compute (modo
--      accumulate) y guarda los campos de rollover en el ciclo. El descuento
--      por rollover lo aplica la capa TS (createInvoiceForCycle).
-- =============================================================================

-- ── 1. Columnas de rollover ──
ALTER TABLE public.monthly_session_cycles
  ADD COLUMN IF NOT EXISTS rollover_mode text NOT NULL DEFAULT 'none'
    CHECK (rollover_mode IN ('none', 'accumulate', 'discount')),
  ADD COLUMN IF NOT EXISTS rollover_sessions_json jsonb,
  ADD COLUMN IF NOT EXISTS rollover_discount_usd numeric(12,2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS rollover_from_period date;

-- ── 2. compute con rollover (sube cuota por servicio) ──
